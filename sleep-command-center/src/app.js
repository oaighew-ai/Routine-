...
function modalHtml(d) {
  ...
  if (type === 'timer-tools') {
    const currentLabel = d.timerType === 'CHECKIN' ? 'CHECK-IN' : d.timerType === 'FEED' ? 'FEED' : (d.state.babyState || 'CURRENT').replaceAll('_',' ');
    return `<div class="modal-wrap" role="dialog" aria-modal="true"><div class="sheet timer-sheet"><div class="eyebrow">Timer controls</div><h2>${esc(currentLabel)}</h2><div class="timer-readout">${fmtElapsed(msSince(d.timerStartedAt,now))}</div><div class="timer-master">IN CRIB · ${fmtElapsed(msSince(d.state.cribPlacedAt,now))}</div><div class="field"><label>Switch current state</label><div class="state-grid"><button data-timer-state="AWAKE_CALM">Awake / calm</button><button data-timer-state="FUSSING">Fussing</button><button data-timer-state="CRYING">Crying</button><button data-timer-state="ASLEEP">Asleep</button></div></div><button class="secondary full" id="pause-for-care">PAUSE FOR CARE</button><button class="secondary full timer-end" id="timer-end-night">END NIGHT / START DAY</button><button class="link-btn" id="cancel-modal">Close</button></div></div>`;
  }
  if (type === 'confirm-end-night') return `<div class="modal-wrap" role="dialog" aria-modal="true"><div class="sheet"><div class="eyebrow">End night</div><h2>Start the day?</h2><p class="micro">This closes the active night and saves the morning summary. If baby is still asleep, keep the night running.</p><button class="primary" id="confirm-end-night">YES · START DAY</button><button class="link-btn" id="cancel-modal">Keep night running</button></div></div>`;
