...
def decide(
    ...
    *,
    ...
    if primary.w_mod <= 0.0:
        ...
        # A market price disagreement is a candidate for research, not
        # evidence that our side is right.  The old path continued into sizing
        # here, which allowed a positive price gap to become BET while the same
        # row said NO_EVIDENCE.  That contradiction is now impossible: until a
        # registered, Stage-A-eligible signal contributes weight, the row is
        # monitor-only with zero stake.
        out.decision = reasons.PASS
        out.stake_units = 0.0
        return out
