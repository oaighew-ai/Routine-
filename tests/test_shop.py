...
class ShopTests(unittest.TestCase):
    ...
    def test_reference_is_the_sharp_book_not_the_soft_median(self):
        """A soft book slower than Pinnacle is the candidate; Pinnacle never is.
        ...
        candidates = [r for r in rows if r.side == "Beta" and r.ev > 0]
        self.assertTrue(candidates, "the price gap should remain visible for research")
        for r in candidates:
    ...
    def test_a_matching_spread_is_compared_on_price(self):
        ...
        better = alpha_at(+115)
        self.assertGreater(better.ev, 0.0)
        self.assertFalse(better.bets)
        self.assertIn(reasons.NO_EVIDENCE, better.decision.reason_codes)
    ...
    def test_a_game_already_under_way_never_bets(self):
        ...
        self.assertTrue([r for r in before if r.ev > 0])
        self.assertFalse([r for r in before if r.bets])
    ...
    def test_one_position_per_game_and_side(self):
        """Price-only rows remain monitor-only until evidence is registered."""
        rows = self.run_shop([
            book("pinnacle", h2h=(-128, +118)),
            book("draftkings", h2h=(-150, +130)),
        ...
        self.assertFalse([r for r in beta if r.bets])
        for r in beta:
            self.assertEqual(r.decision.stake_units, 0.0)
            self.assertIn(reasons.NO_EVIDENCE, r.decision.reason_codes)
