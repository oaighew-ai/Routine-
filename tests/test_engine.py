...
class Check04BNoEvidenceCannotBet(unittest.TestCase):
    """A price gap without registered evidence is monitor-only.

    This is the regression for the live shop board that emitted BET and
    NO_EVIDENCE on the same row.  The authoritative engine must never produce
    that state.
    """

    def test_positive_price_gap_without_evidence_is_pass_zero(self):
        q = Quote(
            event_id="e", sport="ncaaf", market="h2h", side="away",
            line=None, price=130.0, other_price=-150.0,
            consensus_price=118.0, consensus_other_price=-128.0,
        )
        d = decide(q, (), cfg=config.default())
        self.assertGreater(d.ev, 0.0)
        self.assertEqual(d.decision, "PASS")
        self.assertEqual(d.stake_units, 0.0)
        self.assertIn("NO_EVIDENCE", d.reason_codes)
